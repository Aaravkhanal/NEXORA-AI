  ...
  return (
    <div className={`relative w-full ${className}`}>
      <form onSubmit={handleSubmit} className="relative group">
        <div className={`absolute left-0 top-0 bottom-0 flex items-center justify-center text-nexora-text-muted transition-colors group-focus-within:text-nexora-emerald ${large ? 'w-14' : 'w-10'}`}>
          {isLoading ? (
        ...
        <input
          ...
          autoFocus={autoFocus}
          className={`w-full bg-white border-2 border-nexora-offwhite rounded-2xl text-nexora-charcoal placeholder:text-slate-400 focus:outline-none focus:border-nexora-emerald focus:ring-4 focus:ring-nexora-emerald/10 transition-all shadow-sm ${
        ...
        <div className={`absolute right-2 top-2 bottom-2 flex items-center gap-2`}>
          {input.trim() && !isLoading && (
            <span className="hidden sm:flex text-xs font-semibold text-nexora-emerald bg-nexora-warmGreen px-2 py-1 rounded-md items-center gap-1">
          ...
          <button
            ...
            className={`bg-nexora-charcoal hover:bg-nexora-charcoalLight text-white rounded-xl flex items-center justify-center transition-all disabled:opacity-30 disabled:cursor-not-allowed ${
